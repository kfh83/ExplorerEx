...
const WCHAR c_szTaskBandCompositedThemeVert[] = L"TaskBandVertComposited";
...
HRESULT CTaskBand::OnWinEvent(HWND hwnd, UINT dwMsg, WPARAM wParam, LPARAM lParam, LRESULT* plres)
{
    *plres = 0;
    ...
    case WM_SETTINGCHANGE:
        _HandleSettingChange(wParam, lParam, FALSE);
        break;
    ...
    case WM_DWMCOMPOSITIONCHANGED:
        {
            ...
            {
                SendMessageW(ttc, 0x41C, 0, 0);
                SendMessageW(ttc, 0x403, 0, 0xFFFF);
            }
        }
        break;

    case WM_DWMCOLORIZATIONCOLORCHANGED:
        _UpdateThumbnailBackgroundBrush(wParam, FALSE);
        break;
...
private:
    ~TaskShortcut() { CoTaskMemFree(_pszShortcutName); }
...
void CTaskBand::_GetItemTitle(int iIndex, WCHAR* pszTitle, int cchTitle, BOOL fCustom)
{
    *pszTitle = 0;
    ...
    TASKITEM* pti = _GetItem(iIndex, nullptr, TRUE);
    if (pti)
    {
        HWND hwnd = pti->hwnd;
        ...
        if (hwnd)
        {
            if (InternalGetWindowText(hwnd, pszTitle, cchTitle) && fCustom)
            {
                WCHAR szGrpText[260] = L" - ";
                int iIndexGroup = _GetGroupIndex(iIndex);
                _GetItemTitle(iIndexGroup, &szGrpText[3], ARRAYSIZE(szGrpText) - 3, TRUE);
                int iLenGrp = lstrlenW(szGrpText);
                int iLenWnd = lstrlenW(pszTitle);

                if (iLenWnd > iLenGrp && !StrCmpW(&pszTitle[iLenWnd - iLenGrp], szGrpText))
                {
                    pszTitle[iLenWnd - iLenGrp] = 0;
                }
            }
        }
        else if (pti->pszExeName && *pti->pszExeName)
        {
            DWORD cchOut = cchTitle;
            AssocQueryStringW(
                ASSOCF_INIT_BYEXENAME | ASSOCF_VERIFY, ASSOCSTR_FRIENDLYAPPNAME, pti->pszExeName, nullptr, pszTitle, &cchOut);
        }
    }
}
...
int CTaskBand::_InsertItem(HWND hwndTask, PTASKITEM pti, BOOL fForceGetIcon)
{
    ...
    if (!pti)
    {
        ...
        if (_hTheme && IsCompositionActive() && IsWindow(pti->hwnd))
        {
            _RegisterThumbnail(pti->hwnd, &pti->hThumbnail);
        }
    }
...
LRESULT CALLBACK CTaskBand::s_TaskbarSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    switch (uMsg)
    {
    case WM_NOTIFY:
        {
		    NMTBHOTITEM *phot = (NMTBHOTITEM *)lParam;
            if (phot->hdr.code == -521)
            {
                CTaskBand *pTasks = (CTaskBand *)dwRefData;
                if (pTasks->_ShowTooltip(phot->hdr.hwndFrom))
                {
                    return 1;
                }
            }
    ...
    case WM_NCDESTROY:
        {
            RemoveWindowSubclass(hwnd, (SUBCLASSPROC)CTaskBand::s_TaskbarSubclassProc, uIdSubclass);
        }
        break;
    }
...
void CTaskBand::_CreateThumbnailWindows()
{
    ...
    wc.lpszClassName = L"ThumbnailClass";
    RegisterClassW(&wc);

    wc.lpszClassName = L"ThumbnailStackClass";
    RegisterClassW(&wc);

    _hwndThumbStack[0] = SHFusionCreateWindowEx(
        WS_EX_TOPMOST, L"ThumbnailClass", L"", WS_POPUP | WS_DISABLED | WS_SIZEBOX, 0, 0, 0, 0, _hwnd, nullptr,
        g_hinstCabinet, nullptr);

    for (int i = 0; i < _cThumbnails; ++i)
    {
        _hwndThumbStack[1 + i] = SHFusionCreateWindowEx(
            WS_EX_TOPMOST, L"ThumbnailStackClass", L"", WS_POPUP | WS_DISABLED | WS_SIZEBOX, 0, 0, 0, 0,
            _hwnd, nullptr, g_hinstCabinet, nullptr);
        wprintf(L"CTaskBand::_CreateThumbnailWindows: hwndThumbStack[%d] = %p\n", i, _hwndThumbStack[1 + i]);
    }
}

void CTaskBand::_HandleThumbnail(HWND hwnd, const NMTBHOTITEM* pnmHot, bool fGlommed)
{
    _HideThumbnail();
    ...
    if ((pnmHot->dwFlags & HICF_LEAVING) != 0)
    {
        KillTimer(_hwnd, 10);
        KillTimer(_hwnd, 11);
        _fShowThumbnail = FALSE;
    ...
    if (_fShowThumbnail)
    {
        _ShowThumbnail(hwnd, pnmHot->idNew, fGlommed);
    }
    else
    {
        _fPendingThumbnailGlommed = fGlommed;
        _hwndPendingThumbnail = hwnd;
        _idPendingThumbnail = pnmHot->idNew;
        _dwPendingThumbnailTick = _dwTick;
        SetTimer(_hwnd, 10, _dwInitialThumbDelayTime, nullptr);
    }
}
...
{
    CToolTipCtrl ttc = _tb.GetToolTips();
    ttc.SendMessageW(TTM_POP, 0, 0);

    _HideThumbnailWindows();
    TASKITEM* pti = _GetItem(_FindIndexByHwnd(field_140), nullptr, TRUE);
    if (pti)
    {
        DWM_THUMBNAIL_PROPERTIES tnp;
        tnp.dwFlags = DWM_TNP_VISIBLE;
        tnp.fVisible = FALSE;
        DwmUpdateThumbnailProperties(pti->hThumbnail, &tnp);
    }
    field_140 = nullptr;
}
...
{
    // The maximum dimensions of a thumbnail image.
    _sizeThumbnailImage = { 160, 160 };
    SHLogicalToPhysicalDPI(&_sizeThumbnailImage);

    // The margin between a thumbnail and its relative toolbar.
    _sizeThumbnailMargin = { 3, 3 };
    SHLogicalToPhysicalDPI(&_sizeThumbnailMargin);

    // The offset distance between each thumbnail in a group stack.
    _sizeThumbnailGroupOffset = { 3, 3 };
    SHLogicalToPhysicalDPI(&_sizeThumbnailGroupOffset);

    // The height of a thumbnail tooltip.
    _sizeThumbnailTooltip = { 0, 21 };
    SHLogicalToPhysicalDPI(&_sizeThumbnailTooltip);

    // The margin between a thumbnail tooltip and its thumbnail.
    _sizeThumbnailTooltipMargin = { 0, 2 };
    SHLogicalToPhysicalDPI(&_sizeThumbnailTooltipMargin);

    WCHAR szPath[260] = L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer";
    StringCchCatW(szPath, ARRAYSIZE(szPath), L"Thumbnails");

    HUSKEY hkey;
    if (SHRegOpenUSKeyW(szPath, KEY_QUERY_VALUE, nullptr, &hkey, FALSE) == ERROR_SUCCESS)
    {
        DWORD dwType;
        DWORD cbData;
        DWORD dwDefault;
        ...
        dwType = REG_DWORD;
        cbData = sizeof(_dwInitialThumbDelayTime);
        dwDefault = 500;
        SHRegQueryUSValueW(
            hkey, L"InitialThumbnail", &dwType, &_dwInitialThumbDelayTime, &cbData, FALSE, &dwDefault, sizeof(dwDefault));

        dwType = REG_DWORD;
        cbData = sizeof(_dwInitialTooltipDelayTime);
        dwDefault = 1000;
        SHRegQueryUSValueW(
            hkey, L"InitialTooltip", &dwType, &_dwInitialTooltipDelayTime, &cbData, FALSE, &dwDefault, sizeof(dwDefault));

        dwType = REG_DWORD;
        cbData = sizeof(_dwAutoPopTooltipDelayTime);
        dwDefault = 5000;
        SHRegQueryUSValueW(
            hkey, L"AutoPopTooltip", &dwType, &_dwAutoPopTooltipDelayTime, &cbData, FALSE, &dwDefault, sizeof(dwDefault));
    }
    else // Fallback values if the registry keys cannot be read.
    {
        _dwInitialThumbDelayTime = 500;
        _dwInitialTooltipDelayTime = 1000;
        _dwAutoPopTooltipDelayTime = 5000;
    }
}

void CTaskBand::_RegisterThumbnail(HWND hwnd, HTHUMBNAIL* phThumbnail)
{
    (void)hwnd, (void)phThumbnail; // Skipped telemetry ShellTraceID_RegisterThumbnail_Start
    ...
    {
        DWM_THUMBNAIL_PROPERTIES tnp;
        tnp.dwFlags = DWM_TNP_OPACITY | DWM_TNP_SOURCECLIENTAREAONLY;
        tnp.opacity = 255;
...
void CTaskBand::_ShowThumbnail(HWND hwnd, int id, bool fGlom)
{
    (void)id, (void)fGlom; // Skipped telemetry ShellTraceId_Taskbar_ShowThumbnail_Start

    if (_hwndThumbStack[0] && SendMessageW(hwnd, TB_GETHOTITEM, 0, 0) != -1)
    {
        int iButton = id;
        int iIndex = SendMessageW(hwnd, TB_COMMANDTOINDEX, id, 0);
        if (!fGlom)
        {
            iButton = iIndex;
        }

        TASKITEM* pti = _GetItem(iButton, nullptr, TRUE);
        if (pti)
        {
            bool fNoHwnd = pti->hwnd == nullptr;

            TASKITEM* ptiItem;
            TASKITEM* ptiThumbnail;

            if (fNoHwnd)
            {
                int iGroupSize = _GetGroupSize(iButton);
                ptiItem = _GetItem(iButton + iGroupSize, nullptr, TRUE);
                ptiThumbnail = ptiItem;
            }
            else
            {
                ptiThumbnail = pti;
            ...
            if (ptiItem && IsWindow(ptiItem->hwnd))
            {
                field_140 = ptiItem->hwnd;

                RECT rcItem;
                if (SendMessageW(hwnd, TB_GETITEMRECT, iIndex, (LPARAM)&rcItem))
                {
                    RECT rcClient;
                    if (fGlom)
                    {
                    ...
                    SIZE sizeThumbnail = { 0, 0 };
                    DwmQueryThumbnailSourceSize(ptiItem->hThumbnail, &sizeThumbnail);
                    if (sizeThumbnail.cx <= 0 || sizeThumbnail.cy <= 0)
                        return;

                    int cThumbnails = fNoHwnd ? _cThumbnails : 0;

                    RECT rcMargins;
                    SetRectEmpty(&rcMargins);
                    DWORD dwExStyle = GetWindowLongPtrW(_hwndThumbStack[0], GWL_EXSTYLE);
                    DWORD dwStyle = GetWindowLongPtrW(_hwndThumbStack[0], GWL_STYLE);
                    AdjustWindowRectEx(&rcMargins, dwStyle, 0, dwExStyle);

                    double cxScaled;
                    if (sizeThumbnail.cx < sizeThumbnail.cy)
                    {
                        cxScaled = (double)_sizeThumbnailImage.cy * (double)sizeThumbnail.cx / (double)sizeThumbnail.cy + 0.5;
                    }
                    else
                    {
                        cxScaled = (double)_sizeThumbnailImage.cx;
                    }

                    int cxThumb = (int)cxScaled;

                    double cyScaled;
                    if (sizeThumbnail.cy < sizeThumbnail.cx)
                    {
                        cyScaled = (double)_sizeThumbnailImage.cx * (double)sizeThumbnail.cy / (double)sizeThumbnail.cx + 0.5;
                    }
                    else
                    {
                        cyScaled = (double)_sizeThumbnailImage.cy;
                    }

                    int cyWnd = (int)cyScaled + RECTHEIGHT(rcMargins);
                    int cxWnd = cxThumb + RECTWIDTH(rcMargins);

                    int cyThumb = (int)cyScaled;
                    MapWindowPoints(hwnd, nullptr, (POINT*)&rcItem, 2);

                    int iPaddingWidth = cThumbnails * _sizeThumbnailGroupOffset.cx;
                    int cyToolTip = _sizeThumbnailTooltip.cy;

                    UINT uStuckPlace = _ptray->getStuckPlace();
                    if (fGlom && uStuckPlace != STICK_LEFT)
                    {
                    ...
                    switch (uStuckPlace)
                    {
                        case STICK_LEFT:
                        {
                            xPos = rcItem.right + iPaddingWidth + _sizeThumbnailMargin.cx;
                            yPos = rcItem.bottom - cyWnd;
                            break;
                        }
                        case STICK_TOP:
                        {
                            xPos = (rcItem.left + rcItem.right - cxWnd) / 2;
                            yPos = rcItem.bottom + _sizeThumbnailMargin.cy;
                            break;
                        }
                        case STICK_RIGHT:
                        {
                            xPos = rcItem.left - _sizeThumbnailMargin.cx - cxWnd;
                            yPos = rcItem.bottom - cyWnd;
                            break;
                        }
                        default:
                        {
                            xPos = (rcItem.left + rcItem.right - cxWnd) / 2;
                            yPos = rcItem.top - _sizeThumbnailMargin.cy - cyWnd;
                            break;
                        }
                    }

                    RECT rcMonitor;
                    GetMonitorRects(MonitorFromRect(&rcItem, MONITOR_DEFAULTTONEAREST), &rcMonitor, FALSE);

                    int iProjectedXPos = xPos;
                    if (xPos >= rcMonitor.right - cxWnd)
                    {
                        iProjectedXPos = rcMonitor.right - cxWnd;
                    }

                    if (iProjectedXPos > iPaddingWidth + rcMonitor.left)
                    {
                        if (xPos >= rcMonitor.right - cxWnd)
                        {
                            xPos = rcMonitor.right - cxWnd;
                        }
                    }
                    else
                    {
                        xPos = iPaddingWidth + rcMonitor.left;
                    }

                    if (fGlom)
                    {
                        int cx = _sizeThumbnailMargin.cx;
                        if (rcItem.left < cxWnd + cx + xPos)
                        {
                    ...
                    rcClient.right = cxThumb;
                    rcClient.left = 0;
                    rcClient.top = 0;
                    rcClient.bottom = cyThumb;

                    DWM_THUMBNAIL_PROPERTIES tnp;
                    tnp.dwFlags = DWM_TNP_RECTDESTINATION | DWM_TNP_VISIBLE;
                    tnp.rcDestination = rcClient;
                    tnp.fVisible = TRUE;
                    if (SUCCEEDED(DwmUpdateThumbnailProperties(ptiThumbnail->hThumbnail, &tnp)))
                    {
                        _UpdateThumbnailTitle(hwnd, id, cThumbnails);
                        if (cThumbnails > 0)
                        {
                            ...
                            {
                                DeferWindowPos(
                                    hdwp, _hwndThumbStack[0], HWND_TOPMOST, 0, 0, 0, 0,
                                    SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOOWNERZORDER);

                                for (int iThumbnail = cThumbnails; iThumbnail >= 1; --iThumbnail)
                                {
                                    DeferWindowPos(
                                        hdwp, _hwndThumbStack[iThumbnail], _hwndThumbStack[0],
                                        xPos - (iThumbnail * _sizeThumbnailGroupOffset.cx),
                                        yPos - (iThumbnail * _sizeThumbnailGroupOffset.cy),
                                        cxWnd, cyWnd, SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOOWNERZORDER);
                                }
                                EndDeferWindowPos(hdwp);
                            }
                        }
                        ...
                        {
                            SetWindowPos(
                                _hwndThumbStack[0], HWND_TOPMOST, 0, 0, 0, 0,
                                SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOOWNERZORDER);
                        }
                    }
                }
    ...
    (void)id, (void)fGlom; // Skipped telemetry ShellTraceId_Taskbar_ShowThumbnail_Stop
}

void CTaskBand::_UpdateThumbnailBackgroundBrush(COLORREF clrBackground, bool a3)
{
    BOOL fOpaqueBlend;
    if (!a3 || !field_144 && SUCCEEDED(DwmGetColorizationColor(&clrBackground, &fOpaqueBlend)))
    {
        field_144 = 0;

        COLORREF clrBase;
        if (SUCCEEDED(GetThemeInt(_hTheme, 0, 0, TMT_COLORIZATIONCOLOR, reinterpret_cast<int*>(&clrBase))))
        {
            DWORD dwOpacity = LOBYTE(clrBackground >> 24);
            DWORD dwOpacityBlend = 256 - dwOpacity;

            // Calculate the red.
            DWORD dwRed = MulDiv(GetBValue(clrBase), dwOpacityBlend, 256)
                + MulDiv(GetBValue(clrBackground), dwOpacity, 256);

            // Calculate the green.
            DWORD dwGreen = MulDiv(GetGValue(clrBase), dwOpacityBlend, 256)
                + MulDiv(GetGValue(clrBackground), dwOpacity, 256);

            // Calculate the blue.
            DWORD dwBlue = MulDiv(GetRValue(clrBase), dwOpacityBlend, 256)
                + MulDiv(GetRValue(clrBackground), dwOpacity, 256);

            HBRUSH hbrBackground = CreateSolidBrush(RGB(dwRed, dwGreen, dwBlue));
            if (hbrBackground)
            {
                HBRUSH hbrOld = (HBRUSH)SetClassLongPtrW(
                    _hwndThumbStack[0], GCLP_HBRBACKGROUND, reinterpret_cast<LONG_PTR>(hbrBackground));
                if (hbrOld)
                {
                    DeleteObject(hbrOld);
                    field_144 = 1;
...
void CTaskBand::_UpdateThumbnailTitle(const HWND hwnd, WPARAM wParam, int cThumbnails)
{
    ...
    {
		wprintf(L"CTaskBand::_UpdateThumbnailTitle: szTitle = %s\n", szTitle);
        SetWindowText(_hwndThumbStack[0], szTitle);
        if (cThumbnails >= 1)
        {
            for (int i = cThumbnails; i; --i)
            {
                wprintf(L"CTaskBand::_UpdateThumbnailTitle: szTitle = %s\n", szTitle);
...
LRESULT CALLBACK CTaskBand::s_GlomMenuSiteSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    CTaskBand *pThis = (CTaskBand *)dwRefData;
    ...
    switch (uMsg)
    {
        ...
        {
            NMTBHOTITEM *phot = (NMTBHOTITEM *)lParam;
            if (phot->hdr.code == TBN_HOTITEMCHANGE)
            {
                HWND hwndPopup;
                if (SUCCEEDED(IUnknown_GetWindow(pThis->_psmPopup, &hwndPopup)))
                {
                    pThis->_HandleThumbnail(hwndPopup, phot, true);
                }
            }
        }
        break;
        ...
        case WM_NCDESTROY:
        {
            pThis->_HideThumbnail();
            KillTimer(pThis->_hwnd, 10);
            pThis->_fShowThumbnail = FALSE;

            RemoveWindowSubclass(hwnd, (SUBCLASSPROC)s_GlomMenuSiteSubclassProc, uIdSubclass);
        }
        break;

        case WM_PRINTCLIENT:
        {
            if (pThis->_hThemeGlomMenu && IsCompositionActive())
            {
                RECT rc;
                GetClientRect(hwnd, &rc);
                DrawThemeBackground(pThis->_hThemeGlomMenu, (HDC)wParam, 0, 0, &rc, 0);
            }
        }   
        break;
    }
...
BOOL CTaskBand::_ShowTooltip(const HWND hwnd)
{
    BOOL fShown = TRUE;
    ...
    if (_dwInitialTooltipDelayTime + _dwInitialThumbDelayTime <= GetTickCount() - _dwTick)
    {
        if (IsWindowVisible(_hwndThumbStack[0]))
        {
            RECT rcThumbnail;
            GetWindowRect(_hwndThumbStack[0], &rcThumbnail);

            RECT rc;
            GetWindowRect(hwnd, &rc);

            int cxTooltip = rc.right - rc.left;
            int xPos = rcThumbnail.left + (rcThumbnail.right - rcThumbnail.left - (rc.right - rc.left)) / 2;
            int yPos = rcThumbnail.top - _sizeThumbnailTooltipMargin.cy - (RECTHEIGHT(rc));

            RECT rcMonitor;
            GetMonitorRects(MonitorFromRect(&rcThumbnail, MONITOR_DEFAULTTONULL), &rcMonitor, FALSE);

            int iXPosProjected = xPos;
            if (xPos >= rcMonitor.right - cxTooltip)
            {
                iXPosProjected = rcMonitor.right - cxTooltip;
            }
            if (rcMonitor.left > iXPosProjected)
            {
                xPos = rcMonitor.left;
            }
            else if (xPos >= rcMonitor.right - cxTooltip)
            {
                xPos = rcMonitor.right - cxTooltip;
            }
            SetWindowPos(hwnd, nullptr, xPos, yPos, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOOWNERZORDER);
        }
    }
    else
    {
        SetWindowPos(hwnd, nullptr, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
        field_120 = hwnd;
        DWORD dwTick = GetTickCount();
        SetTimer(_hwnd, 11, _dwInitialTooltipDelayTime + _dwTick + _dwInitialThumbDelayTime - dwTick, nullptr);
    }
    return fShown;
}

LRESULT CALLBACK CTaskBand::s_GlomMenuToolbarSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    switch (uMsg)
    {
        case WM_NOTIFY:
        {
            ...
            {
                CTaskBand *pThis = (CTaskBand *)dwRefData;
                if (!pThis->_ShowTooltip(phot->hdr.hwndFrom))
                {
                    return 1;
                }
            }
        ...
        case WM_NCDESTROY:
        {
            RemoveWindowSubclass(hwnd, s_GlomMenuToolbarSubclassProc, uIdSubclass);
        }
        break;
    }
...
void CTaskBand::_SetGlomMenuTheme(HWND hwnd)
{
    if (_hThemeGlomMenu)
    {
                                ...
                                if (SUCCEEDED(IUnknown_GetWindow(punk, &hwndGlomMenu)))
                                {
                                    ...
                                    if (_hThemeGlomMenu && IsCompositionActive())
                                    {
                                        ...
                                        if (hwndRoot)
                                        {
                                            ...
                                            DwmEnableBlurBehindWindow(hwndRoot, &blurBehind);
                                        }
                                    }
                                }
                                punk->Release();
...
void CTaskBand::_HandleGetMinRect(HWND hwndShell, POINTS * prc)
{
    ...
    // make sure the rcDestination is within out client area
...
BOOL _IsRudeWindow(HMONITOR hmon, HWND hwnd, HMONITOR hmonTask, BOOL fSkipActiveCheck)
{
    ...
    if (IsWindowVisible(hwnd) && hwnd != v_hwndDesktop)
    {
        ...
        // The old logic was:
        //
        // If the app's window rcDestination takes up the whole monitor, then it's rude.
        ...
        //
        // Otherwise, get the client rcDestination rather than the window rcDestination
        // and compare that rcDestination against the monitor rcDestination.
        //
        ...
        {
            // Doesn't match rude app profile; use client rcDestination
            GetClientRect(hwnd, &rcApp);
            MapWindowPoints(hwnd, HWND_DESKTOP, (LPPOINT)&rcApp, 2);
        }
        else
        {
            // Matches rude app profile; use window rcDestination
...
void CTaskBand::_OnWindowActivated(HWND hwnd, BOOL fSuspectFullscreen)
{
    ...
    SetTimer(_hwnd, 5, 1000, nullptr);

    if (!pti)
    {
        int fFoundBackup = 0;
        int iMax = _tb.SendMessageW(0x418, 0, 0);
        for (int i = 0; i < iMax; ++i)
        {
            TASKITEM* ptiT = _GetItem(i, nullptr, TRUE);
            if (ptiT && ptiT->hwnd)
            {
                DWORD dwFlags = ptiT->dwFlags;
                if ((dwFlags & 4) != 0 || !fFoundBackup && (dwFlags & 8) != 0)
                {
                    DWORD dwpid1;
...
LRESULT CTaskBand::_HandleShellHook(int iCode, LPARAM lParam)
{
    ...
    case HSHELL_TASKMAN:
        if (!_ptray->_fStuckRudeApp && GetAsyncKeyState(VK_CONTROL) >= 0)
        {
            ...
            {
                _ClosePopupMenus();
                SHAllowSetForegroundWindow(_hwndPrevFocus);
                SetForegroundWindow(_hwndPrevFocus);
                _hwndPrevFocus = nullptr;
                return TRUE;
            }
        }
        PostMessageW(v_hwndTray, TM_ACTASTASKSW, 0, 0);
        return TRUE;
...
//---------------------------------------------------------------------------
LRESULT CTaskBand::_HandleSettingChange(WPARAM wParam, LPARAM lParam, BOOL fOnCreate)
{
    _tb.SendMessage(WM_SETTINGCHANGE, wParam, lParam);
        ...
        case 10:
            {
                ...
                if (!field_128
                    && _dwPendingThumbnailTick == _dwTick
                    && !c_tray._stb.IsPopupMenuVisible()
                    && _hTheme
                    && _CanShowThumbnail())
                    {
                        _fShowThumbnail = TRUE;
					    _ShowThumbnail(_hwndPendingThumbnail, _idPendingThumbnail, _fPendingThumbnailGlommed);
                    }
                }
                break;
            }
    ...
    case 0x43F:
        {
            ...
            else
            {
                ...
                {
                    SendMessage(hwnd, 0x403, 0, 0xFFFF);
                }
            }
            return 0;
        }
